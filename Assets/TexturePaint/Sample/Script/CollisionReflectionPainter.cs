...
namespace Es.TexturePaint.Sample
{
	...
	public class CollisionReflectionPainter : MonoBehaviour
	{
		...
		public void OnCollisionStay(Collision collision)
		{
			...
			if(!collision.contacts.Any(p => p.otherCollider.GetComponent<DynamicCanvas>() != null))
				return;

			cam.transform.position = transform.position + offset;

			var contact = collision.contacts.First(p => p.otherCollider.GetComponent<DynamicCanvas>() != null);
			var canvas = contact.otherCollider.GetComponent<DynamicCanvas>();
